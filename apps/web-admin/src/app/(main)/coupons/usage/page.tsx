    ...
    <div className="space-y-6 py-5 px-2.5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
            <span className="h-8 w-8 rounded-lg bg-[#239459] text-white flex items-center justify-center"><TrendingUp className="w-4 h-4" /></span>
            Coupon Usage
          </h1>
      ...
      {stats ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <KpiCard title="Redemptions" value={String(stats.filteredTotal)} sub="matching filters" icon={<Ticket className="w-5 h-5 text-white" />} iconBg="bg-[#239459]" />
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] p-3 shadow-sm">
        ...
        {activeFilterCount > 0 && !showFilters && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {couponId.trim() && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#239459]/10 dark:bg-[#239459]/15 text-[#8a5f17] dark:text-[#239459] rounded-full text-xs font-medium border border-[#239459]/30 dark:border-[#239459]/30">Coupon: #{couponId.trim()} <button onClick={() => setCouponId('')}><X className="w-3 h-3" /></button></span>}
