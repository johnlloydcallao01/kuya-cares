    ...
    <div className="space-y-6 py-5 px-2.5">
      ...
      {stats || pagination || !loading ? (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <KpiCard title="Total Items" value={String(kpiTotalItems)} sub={`${stats?.totalAll ? `${stats.totalAll} overall` : `${pagination?.totalDocs ?? kpiTotalItems} in DB`}`} icon={<Package className="w-5 h-5 text-white" />} iconBg="bg-[#239459]" />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    const active = sort === v
                    return <button key={v} onClick={() => setSort(v)} className={`px-2.5 py-1 rounded-full text-xs font-medium border ${active ? 'bg-[#239459] text-white border-[#239459]' : 'bg-white dark:bg-[#0a0a0a] text-gray-700 dark:text-[#a1a1aa] border-gray-200 dark:border-[#262626]'}`}>{l}</button>
                  })}
                </div>
              </div>
              <div>
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        ...
        {showTableSkeleton ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            <div className="h-16 w-16 bg-[#239459]/10 dark:bg-[#239459]/15 rounded-2xl flex items-center justify-center mb-4"><ShoppingBag className="w-8 h-8 text-[#239459]" /></div>
            ...
            <Link href="/orders" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#239459] text-white rounded-lg text-sm font-semibold"><Receipt className="w-4 h-4" /> View orders</Link>
          </div>
        ) : !error && (
          <>
                      ...
                      <tr key={String(it.id)} className="hover:bg-gray-50 dark:hover:bg-[#0a0a0a]/50 transition">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1 min-w-[140px]">
                            <Link href={`/orders/${String(orderId)}`} className="font-mono text-xs font-semibold text-gray-900 dark:text-white hover:text-[#239459] hover:underline">#{String(orderId)}</Link>
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell">
                          <div className="text-xs font-medium text-gray-900 dark:text-white truncate max-w-[160px] flex items-center gap-1"><Store className="w-3 h-3 text-[#239459]" /> {productName}</div>
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-full text-xs font-semibold text-gray-700 dark:text-white">
                            <Layers className="w-3 h-3 text-[#239459]" /> × {it.quantity}
                          </span>
                        </td>
                            ...
                            <div className="flex flex-wrap gap-1 max-w-[180px]">
                              {(() => {
                                ...
                                return entries.slice(0, 3).map(([k, v]) => (
                                  <span key={k} className="inline-flex px-1.5 py-0.5 rounded-full text-[11px] font-medium bg-[#239459]/10 text-[#8a5f17] dark:text-[#239459] border border-[#239459]/20 truncate max-w-[100px]">{k}: {String(v).slice(0, 20)}</span>
                                ))
                              })()}
                              {(() => {
                                const snap = it.options_snapshot as any
