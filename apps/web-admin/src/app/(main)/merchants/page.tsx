                    ...
                    <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-[#262626] dark:hover:bg-[#0a0a0a]/50 transition">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3 min-w-[220px]">
                          {m.vendor?.logo?.url ? (
                            <img src={m.vendor.logo.url} alt={m.vendor.businessName || m.outletName} className="h-9 w-9 rounded-xl object-cover shrink-0" />
                          ) : (
                            <div className="h-9 w-9 rounded-xl bg-gray-100 dark:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden">{initials(m.vendor?.businessName || m.outletName)}</div>
                          )}
